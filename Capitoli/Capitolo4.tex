...
\end{equation}
\label{hc}
con $n$ il numero totale delle pagine, e $n_c$ ed $n_k$ il numero delle pagine appartenenti alla classe $c$ o al cluster $k$, ed infine $n_{c,k}$ sono le pagine della classe $c$ assegnate al cluster $k$. In una situazione ideale l'omogeneità assume valore 1.  \color{red}
cita: V-Measure: A conditional entropy-based external cluster evaluation
measure 
\color{black}
...
\item \textbf{Adjusted Rand Index (ARI)}: nota la ground truth e gli assegnamenti restituiti da un algoritmo di clustering, l'ARI calcola la similarità tra i due insiemi ignorando le permutazioni. 
...
Essendo una misura probabilistica i valori possibili spaziano nell'intervallo $(0,1)$. 
...
\color{red} Fra i vantaggi è degno di nota che nessuna assunzione viene fatta sulla struttura dei cluster, quindi possono essere utilizzate con algoritmi che identificano cluster di forma diversa. NON SI CAPISCE BENE PERCHE QUESTO E' UN VANTAGGIO SOLO PER L MI
\color{black}
\paragraph{Silhouette} misura la forza di ogni cluster. Essa può essere usata per calcolare la qualità dei cluster anche quando la Ground Truth non è nota.
Formalmente la Silhouette può essere così definita:
\begin{equation}
s = \frac{b - a}{\max (a,b)}
\end{equation}
dove:
\color{controlla semantica di questo elenco}
\begin{itemize}
...
\color{red} incorpora questa sezione nell'elenco puntato ad inizio capitolo
\color{black}
